  ...
  try {
    ...
    if (pathname === '/api/friends/request' && req.method === 'POST') {
      return body(async (body_) => {
        ...
        if (sa.friends.includes(them)) return json(409, { error: 'Already friends' });
        if (sa.out.some(x => x.to === them)) return json(409, { error: 'Request already sent' });
        if (sb.in.some(x => x.from === me)) return json(409, { error: 'Request already pending' });
        const ts = Date.now();
        sa.out.push({ to: them, ts });
        sb.in.push({ from: me, ts });
        await saveSoc(me, sa);
