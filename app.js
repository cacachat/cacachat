...
function enterApp() {
  ...
  loadDMConvos();
  loadSocialData();
  drainFriendRequestQueue(); // deliver any offline-queued friend requests
  loadServers();
  setupProfile();
  buildEmojiPicker();
  buildThemeGrid();
...
// Called on login — processes any friend requests that were queued
// while this account was offline (written by the sender's browser).
function drainFriendRequestQueue() {
  if (!me) return;

  // ── Drain incoming friend requests ──
  ...
  if (q.length) {
    let changed = false;
    q.forEach(req => {
      req.from = (req.from || '').toLowerCase(); // normalize
      ...
      pendingIn.push({
        from: req.from,
        displayName: req.displayName || req.from,
    ...
    if (changed) saveSocialData();
    localStorage.removeItem(queueKey);
  }

  // ── Drain acceptances (someone accepted our request while we were offline) ──
  const aqKey = 'cc_friendaccept_inbox_' + me.name;
  let aq = [];
  try { aq = JSON.parse(localStorage.getItem(aqKey) || '[]'); } catch {}
  if (aq.length) {
    aq.forEach(u => {
      u.name = (u.name || '').toLowerCase(); // normalize
      pendingOut = pendingOut.filter(p => p.to !== u.name);
      if (!friends.find(f => f.name === u.name)) {
        friends.push({ name: u.name, displayName: u.displayName || u.name, color: u.color || 0, avatar: u.avatar || null, pfp: u.pfp || null, banner: u.banner || null, bio: u.bio || '' });
      }
      // Cache profile data locally
      try {
        if (u.pfp) localStorage.setItem('cc_pfp_' + u.name, u.pfp);
        if (u.banner) localStorage.setItem('cc_banner_' + u.name, u.banner);
        if (u.bio) localStorage.setItem('cc_bio_' + u.name, u.bio);
      } catch {}
    });
    saveSocialData();
    localStorage.removeItem(aqKey);
  }
  ...
  switch (data.type) {
    ...
    case 'friend_accepted':
      ...
      if (data.to && (data.to || '').toLowerCase() !== me.name) break;
      {
        ...
        saveSocialData();
        // Always refresh the UI — switch to friends tab so sender sees the new friend
...
async function sendFriendRequest() {
  ...
  // ── API call: validates user exists AND delivers server-side ──
  let apiRes = null;
  try {
  ...
  if (apiRes && apiRes.error) {
    // Map common server error messages to friendly text
  ...
  // If API is unreachable (network error), fall back to online-user check
  if (!apiRes) {
    const isOnline = onlineUsers.find(u => u.name === username);
    if (!isOnline) {
      // Can't confirm they exist — warn the user but don't silently pretend it worked
      errEl.textContent = 'Could not reach the server. Make sure @' + username + ' exists and try again.';
      errEl.classList.add('show');
      return;
    }
    // They're online right now, WS delivery will work
  }
...
function acceptFriend(fromName) {
  ...
  // Cache their profile for local lookups
  try {
  ...
  try {
    const queueKey = 'cc_friendreq_inbox_' + me.name;
    let q = []; try { q = JSON.parse(localStorage.getItem(queueKey) || '[]'); } catch {}
  ...
  // ── Write into the SENDER's acceptance-queue so they get it even if offline ──
  const myProfile = {
    name: me.name,
    displayName: me.displayName || me.name,
    color: AVATAR_COLORS.indexOf(selectedColor),
  ...
  try {
    const aqKey = 'cc_friendaccept_inbox_' + fromName;
    let aq = []; try { aq = JSON.parse(localStorage.getItem(aqKey) || '[]'); } catch {}
    if (!aq.find(r => r.name === me.name)) {
      aq.push(myProfile);
      localStorage.setItem(aqKey, JSON.stringify(aq));
    }
  } catch {}

  // ── Live WS relay for instant delivery if sender is currently online ──
...
function cancelFriendRequest(toName) {
  ...
  saveSocialData();
  // Remove from the recipient's offline queue too
  try {
